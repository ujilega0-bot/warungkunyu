...
const FIREBASE_DATABASE_URL = window.WARUNGKU_FIREBASE_DATABASE_URL || '';
...
async function savePosOrderOnline(posOrder) {
  if (!isFirebaseConfigured()) return false;

  const url = `${getFirebaseDatabaseUrl()}/${POS_ONLINE_ORDERS_PATH}/${encodeURIComponent(posOrder.id)}.json`;
...
async function sendOrderToPOS(receiptData) {
  ...
  try {
    const syncedOnline = await savePosOrderOnline(posOrder);
    window._lastPosOrderSyncedOnline = syncedOnline;
    return syncedOnline;
  } catch (error) {
    console.warn('Order belum bisa dikirim ke POS online:', error);
    window._lastPosOrderSyncedOnline = false;
    return false;
  }
}
...
async function buildReceipt() {
  ...
  return sendOrderToPOS(window._receiptData);
}
...
function buildAdminOrderMessage(order) {
  ...
  return [
    ...
    '',
    '*STRUK 1 - DETAIL PESANAN*',
    '====================',
    `No. Order: ${order.orderNo}`,
    `Tanggal: ${order.dateStr || '-'}`,
    `Waktu: ${order.timeStr || '-'}`,
    `Toko: ${order.storeName || STORE_NAME}`,
    `Kasir: ${order.cashierName || CASHIER_NAME}`,
    '',
    '*DATA PELANGGAN*',
    `Nama: ${order.name || '-'}`,
    ...
    '',
    '*STRUK 2 - PERMINTAAN BANTUAN / TAMBAHAN*',
    '====================',
    `No. Order: ${order.orderNo}`,
    `Nama: ${order.name || '-'}`,
    `WhatsApp: ${order.phone || '-'}`,
    ...
    '',
    '*TINDAKAN ADMIN*',
    '- Cek apakah ada permintaan tambahan.',
    '- Konfirmasi ke pelanggan jika ada perubahan harga/stok.',
    '- Proses pesanan sesuai detail struk 1.',
    '',
...
$('confirmPayBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    updateOrderCount();
    closeCheckout();
    const syncedOnline = await buildReceipt();

    showToast(syncedOnline
